...
description: >-
  ...
  to that agent (worker_label, e.g. opencode), claims it, does the work, writes
  ...
  — e.g. "work one opencode task on Build", "have the agent pick up the next
  open task on Payments and do it". This is the same flow each scheduled tick
  of lc-start-job runs, just invoked once. Does not create a recurring job
...
recurring schedule, and it does **not** create a recurring job.
...
up a recurring schedule. Examples: "work one opencode task on Build", "have the
agent pick up the next open task on Payments and do it", "do one tick of work
on the Launch effort for opencode".

## When NOT to use this skill

- The user wants this to keep happening **unattended on a schedule** (every 5
  minutes) → use `lc-start-job`, which sets up a recurring launchd job.
...
`in_progress`. No recurring job is created by this skill, so there is nothing to
"stop".

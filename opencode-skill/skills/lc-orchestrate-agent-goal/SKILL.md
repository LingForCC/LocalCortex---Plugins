...
**driver**. `lc-orchestrate-agents` installs a **macOS launchd LaunchAgent**
...
fresh headless worker that runs the **`lc-start-work`** skill for a specific
**task id**.
...
flow. This skill installs **no LaunchAgent** and creates no scheduled job —
...
fresh headless workers* rather than doing it in-process. Examples: "orchestrate
...
LaunchAgent lingering in the background. Prefer `lc-orchestrate-agents` when
you want unattended background polling that keeps catching future tasks over
hours or days.
...
- The user wants **unattended, recurring** delegation that keeps running in the
  ...
  which installs a 5-minute launchd LaunchAgent.
...
**Do not install a LaunchAgent.** This skill creates no launchd job — there is
no scheduled automation in this flow. Instead,
...
> into a launchd job — it is the loop **you (the orchestrator agent) run right
...
worker per such agent. After `wait`ing for all of them, **start the next round**
...
and collect each exit code. One task per agent per round — each worker's
...
- **Non-zero exit** → that worker failed. **Do not retry it in this round**, and
  do not complete anything on its behalf — its task is still `open` (or
  `in_progress` if it crashed mid-work), and the next round will reconsider it.
...
- **It runs to completion, then stops on its own.** Unlike
  `lc-orchestrate-agents` (whose idle ticks fire forever until the user removes
  the LaunchAgent), this loop keeps going until **no supported agent has any
  ...
  keeps the session alive so it can resume the moment a blocker is resolved or
  a new task is added, rather than forcing the user to re-invoke.
...
delete (contrast with `lc-orchestrate-agents`, which installs a recurring
5-minute LaunchAgent). Remind the user the spawned worker CLIs must stay logged
in for their rounds to do real work, and that **the agent roster is re-read
each round**, so editing agents in the app takes effect on the next round
without re-running setup.

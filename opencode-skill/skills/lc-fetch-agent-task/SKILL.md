...
description: >-
  ...
  (the `worker_label`, e.g. `opencode`); the skill returns the active tasks
  ...
  in effort Y", "find the tasks assigned to opencode in …", or "show me the
...
List the tasks assigned to the agent labeled with the given **agent label**
within the Effort whose id the user provides, and report their ids, names, and
status. Drive LocalCortex **exclusively through its JXA/AppleScript surface**
...
and an **agent label** (e.g. `opencode`, `kimi`, `claude`), and wants to see the
tasks assigned to that agent inside that effort — e.g. "what is opencode working
...
```json
{
  "query": { "effort_id": "…", "agent_label": "opencode" },
  "count": 1,
  "tasks": [
    {
      "id": "…",
...
2. **Confirm the inputs:** the **effort id** and the **agent label**. If the
   ...
   The agent label is the worker's `worker_label` (e.g. `opencode`), not the

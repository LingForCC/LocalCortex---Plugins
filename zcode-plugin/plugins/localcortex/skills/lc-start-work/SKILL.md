...
description: >-
  ...
  to that agent (worker_label, e.g. zcode), claims it, does the work, writes
  ...
  — e.g. "work one zcode task on Build", "have the agent pick up the next open
  task on Payments and do it". This is the same flow each scheduled tick of
  lc-start-job runs, just invoked once. Does not create a ZCode automation and
  does not chain sibling skills.
...
recurring 5-minute schedule, and it does **not** create a ZCode automation.
...
up a recurring schedule. Examples: "work one zcode task on Build", "have the
agent pick up the next open task on Payments and do it", "do one tick of work
on the Launch effort for zcode".
...
- The user points at a **specific task** by id or name and wants to work on
  *that* one (with the follow-up collection flow) → use `start-work`. This
  skill picks the next open task itself; it is not for a caller-chosen task.
...
- **Do not create follow-up tasks.** This run completes the open task it picked
  up; it does not run the `start-work` follow-up flow. If a task clearly needs
...
`in_progress`. No automation is created by this skill, so there is nothing to
"stop".

...
description: >-
  ...
  (the `worker_label`, e.g. `zcode`); the skill returns the active tasks
  ...
  in effort Y", "find the tasks assigned to zcode in …", or "show me the
  agent-owned tasks for …". Read-only; does not read full notes or modify
  tasks. For starting/working a returned task, use the lc-start-work skill.
...
List the tasks assigned to a specific **agent** (by its `worker_label`) within
a given **Effort**, and report their ids, names, and status. Drive LocalCortex
...
and an **agent label** (e.g. `zcode`, `claude`, `codex`), and wants to see the
tasks assigned to that agent inside that effort — e.g. "what is zcode working
...
returned task, hand off to the `lc-start-work` skill with that task's id.
...
# the helper is under skills/lc-fetch-agent-task/scripts/lc.js.
if [ -n "$ZCODE_PLUGIN_ROOT" ]; then
  LC_JS="$ZCODE_PLUGIN_ROOT/skills/lc-fetch-agent-task/scripts/lc.js"
else
...
```json
{
  "query": { "effort_id": "…", "agent_label": "zcode" },
  "count": 1,
  "tasks": [
    {
      "id": "…",
...
- The returned `tasks` carry the **summary** fields (no `notes`); each has a
  `has_notes` hint. Use `tasks-get <taskId>` directly if you
...
2. **Confirm the inputs:** the **effort id** and the **agent label**. If the
   ...
   The agent label is the worker's `worker_label` (e.g. `zcode`), not the
...
hand off to `lc-start-work` with that task's id.

/*
 * lc.js — JXA helper for the LocalCortex `lc-complete-task` skill.
 *
 * Pure JXA (no Node). Driven by `osascript -l JavaScript <path>/lc.js <cmd>`.
 * A focused subset of the LocalCortex AppleScript surface
 * (see LocalCortex.sdef in the LocalCortex---Swift repo): `complete task`,
 * `get task`, and `workspace path`. `complete task` is the point of this skill;
 * `get task` is bundled so the skill can read a task before/after completing
 * (e.g. to confirm the transition and report it) without leaving the skill;
 * `workspace path` lets it resolve an effort's workspace folder when the
 * caller wants to write a completion artifact.
 *
 * The only optional input here is the `completed` flag (env var LC_COMPLETED,
 * defaults to true on the app side when omitted). The task id and the
 * subcommand travel as argv. Every command prints the app's JSON-string result
 * to stdout (so the caller JSON.parse's it).
 *
 * Errors:
 *   - Any failure — bad usage, or an app-level failure (app not installed:
 *     -2700; validation -1001 e.g. an incomplete blocker blocks completion;
 *     not_found -1002; conflict -1003) — is surfaced by throwing, which
 *     osascript turns into a non-zero exit with the message on stderr. The
 *     caller reads stderr on non-zero exit. On success, the app's JSON-string
...
function run() {
  ...
  switch (cmd) {
    ...
    case "task-complete": {
      ...
      // forward the key when the caller set it explicitly. LC_COMPLETED=false
      // reopens the task; LC_COMPLETED=true completes it (same as omitting).
    ...
    default:
      throw new Error(
        "unknown subcommand: '" + cmd + "'. Expected one of: " +
          "tasks-get, workspace-path, task-complete."
      );
  }
...
// bad usage, or an app/AppleScript failure surfaced by the Application call —
// propagates as a non-zero exit with the message on stderr (osascript prints

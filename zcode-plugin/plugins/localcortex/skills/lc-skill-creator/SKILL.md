...
description: >-
  Create or revise a ZCode skill that drives the LocalCortex macOS task manager
  app through its JXA/AppleScript automation surface (osascript). The
...
if [ -n "$ZCODE_PLUGIN_ROOT" ]; then
  LC_JS="$ZCODE_PLUGIN_ROOT/skills/lc-skill-creator/scripts/lc.js"
else
...
2. **`SKILL.md` frontmatter** — `name`, `description` (keep under 1024 chars),
   `argument-hint`, `allowed-tools` (the ZCode tool names the skill may use,
...
9. **List the new skill** in both READMEs (`zcode-plugin/README.md` and
   `zcode-plugin/plugins/localcortex/README.md`) and bump the plugin version in
   `marketplace.json` **and** `.zcode-plugin/plugin.json` (keep them in sync).
...
6. **Update the plugin:** add the skill to both READMEs' skill lists, and bump
   the version in `marketplace.json` **and** `.zcode-plugin/plugin.json`
   together.

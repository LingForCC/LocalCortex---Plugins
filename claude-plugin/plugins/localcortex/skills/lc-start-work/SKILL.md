...
description: >-
  ...
  tick hands each spawned worker a specific task id to work. Does not create a
  recurring Claude Code automation and does not chain sibling skills.
...
task** per invocation. This is exactly what each worker spawned by
`lc-orchestrate-agents` runs; this skill runs it **once, right now**, and
does **not** create a recurring Claude Code automation.
...
- The user wants this to keep happening **unattended on a schedule** across
  app-defined agents → use `lc-orchestrate-agents`.
...
if [ -n "$CLAUDE_PLUGIN_ROOT" ]; then
  LC_JS="$CLAUDE_PLUGIN_ROOT/skills/lc-start-work/scripts/lc.js"
else
...
new task id), or use `lc-orchestrate-agents` for recurring multi-agent
delegation. Report what you did.
...
group just finished, the report ends with the parent close-out line (Step 6). No automation is created

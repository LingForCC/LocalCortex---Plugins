...
description: >-
  Create or revise a Claude Code skill that drives the LocalCortex macOS task
  ...
  — list efforts/templates/tasks/agents, get task, workspace path, create/update/
  complete task, create/update/delete agent — plus the record DTO shapes, the
  env-var/argv calling convention, the bundled lc.js helper pattern, and an
  honest account of what JXA automation CANNOT do (no name search, no date or
  recurrence clearing, templates read-only, file bytes never cross the wire,
  efforts are UI-only, etc.). Use when the user wants to build a new lc-* skill,
  automate a LocalCortex workflow the existing skills don't cover, or learn the
  JXA surface to hand-roll automation. Generated skills copy this skill's bundled
  lc.js and trim it to the commands they need.
...
`lc.js` lives next to this `SKILL.md`, in `scripts/`. Prefer the host-provided
plugin root; fall back to this skill's directory (the parent of this
`SKILL.md`).

```bash
# Resolve once. CLAUDE_PLUGIN_ROOT points at the plugin root (…/localcortex);
# the helper is under skills/lc-skill-creator/scripts/lc.js.
if [ -n "$CLAUDE_PLUGIN_ROOT" ]; then
  LC_JS="$CLAUDE_PLUGIN_ROOT/skills/lc-skill-creator/scripts/lc.js"
else
...
1. **One skill = one folder** under
   `plugins/localcortex/skills/<skill-name>/` containing `SKILL.md` and
   `scripts/lc.js`. The skill name is `lc-<verb>` (kebab-case).

2. **`SKILL.md` frontmatter** — `name`, `description` (keep under 1024 chars),
   `argument-hint`, `allowed-tools` (the Claude Code tool names the skill may use,
   e.g. `[Bash, Read, Write, Edit]`), `version`, `license`. The `description`
   is what the host uses to decide when to load the skill — make it specific
   about *when to use* and *when not to use*.
...
4. **Free text via env vars; UUIDs + subcommand via argv.** Never inline a task
   name, notes, or prompt in argv. Reuse the env-var names (`LC_NAME`,
   `LC_NOTES`, `LC_STATUS`, `LC_WORKER`, `LC_AGENT_ID`, `LC_BLOCKERS`, …) so the
   surface is consistent across skills.
...
8. **TCC prompt.** Tell the user to expect a one-time "… wants to control
   LocalCortex" prompt on the first call from the host binary.

9. **List the new skill** in both READMEs (`claude-plugin/README.md` and
   `claude-plugin/plugins/localcortex/README.md`) and bump the plugin version in
   `marketplace.json` **and** `.claude-plugin/plugin.json` (keep them in sync).
...
4. **Scaffold the skill folder:**
   `plugins/localcortex/skills/<skill-name>/SKILL.md` and
...
6. **Update the plugin:** add the skill to both READMEs' skill lists, and bump
   the version in `marketplace.json` **and** `.claude-plugin/plugin.json`
   together.

7. **Report** what was created (folder path, commands used, version bump), and
   call out any "cannot do" items that shaped the design.
...
Report plainly: the new skill's name and folder, the sdef commands it uses, the
plugin version you bumped to, and any "cannot do" limitations that shaped the
design (with the closest alternative you chose instead). If you only wrote a
one-off automation rather than a packaged skill, say that and show the commands.

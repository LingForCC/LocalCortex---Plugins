...
Set up a scheduled Claude Code automation that, every 5 minutes, checks a named
...
1. **At setup time (interactive, with the user):** collect the effort name and
   ...
   app** (`list agents`) and map each to a spawnable CLI; create the Claude Code
...
- **Claude Code scheduled automations** are enabled in this host. The setup step
...
if [ -n "$CLAUDE_PLUGIN_ROOT" ]; then
  LC_JS="$CLAUDE_PLUGIN_ROOT/skills/lc-orchestrate-agents/scripts/lc.js"
else
...
### Step 4 — Create the Claude Code automation (every 5 minutes)
...
recurring automation will pick up future tasks.
...
(`CronDelete`), or ask you to stop it. Also tell them the automation keeps
firing even when no agent has any active work left — idle ticks are silent
no-ops that create nothing, and the automation only stops when they delete it.
Also
...
create no task** — idle ticks are silent no-ops. The automation cannot delete
or disable itself, so it simply keeps firing no-op ticks until the user deletes
it. Skip step 5 entirely and stop.
...
nothing — the automation keeps firing until the user deletes it. Finally note

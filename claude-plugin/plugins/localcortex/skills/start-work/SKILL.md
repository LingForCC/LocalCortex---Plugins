...
`$LC_JS` for every call. Claude Code substitutes `$CLAUDE_PLUGIN_ROOT` with the
installed plugin's root directory; the helper is under
`skills/start-work/scripts/lc.js` relative to it.
...
if [ -n "$CLAUDE_PLUGIN_ROOT" ]; then
  LC_JS="$CLAUDE_PLUGIN_ROOT/skills/start-work/scripts/lc.js"
else
...
```bash
LC_STATUS=in_progress LC_WORKER=agent LC_WORKER_LABEL=claude \
  osascript -l JavaScript "$LC_JS" task-update "$TASK_ID"
```

Then **re-read the task** (`tasks-get`) and confirm its `worker` is `agent`
**and** `worker_label` is `claude`. If it isn't, **stop and double-check with
...
Follow-up: write integration tests (owner: agent/claude).' \
  osascript -l JavaScript "$LC_JS" task-update "$TASK_ID"
```

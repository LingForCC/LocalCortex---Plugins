...
`lc.js` lives in the `scripts/` directory next to this `SKILL.md`. Resolve its
absolute path from the installed skill directory once and reuse `$LC_JS` for
every call.
...
```bash
LC_STATUS=in_progress LC_WORKER=agent LC_WORKER_LABEL=codex \
  osascript -l JavaScript "$LC_JS" task-update "$TASK_ID"
```

Then **re-read the task** (`tasks-get`) and confirm its `worker` is `agent`
**and** `worker_label` is `codex`. If it isn't, **stop and double-check with
...
Follow-up: write integration tests (owner: agent/codex).' \
  osascript -l JavaScript "$LC_JS" task-update "$TASK_ID"
```

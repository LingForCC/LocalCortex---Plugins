/*
 * lc.js — JXA helper for the LocalCortex `start-work` skill.
 *
 * Pure JXA (no Node). Driven by `osascript -l JavaScript <path>/lc.js <cmd>`.
 * It wraps the seven LocalCortex AppleScript commands (see LocalCortex.sdef
 * in the LocalCortex---Swift repo), which have parity with the MCP tools.
 *
...
function run() {
  ...
  switch (cmd) {
    ...
    case "task-create": {
      ...
      // Only set keys the sdef treats as optional when the caller provided a
      // value; absent keys mean "unchanged"/"default" on the app side too.
    ...
    default:
      throw new Error(
        "unknown subcommand: '" + cmd + "'. Expected one of: " +
          "efforts-list, tasks-list, tasks-get, workspace-path, " +
...
// again here makes the whole script (and every command, including
// task-create) run TWICE.

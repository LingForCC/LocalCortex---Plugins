...
description: >-
  Create or revise a Codex skill that drives the LocalCortex macOS task manager
  ...
  pattern, and an honest account of what JXA automation cannot do. Use when the
  user wants to build a new lc-* skill, automate a LocalCortex workflow the
  existing skills do not cover, extend an existing lc-* skill, or learn the JXA
  surface to hand-roll automation. Generated skills copy this skill's bundled
...
- The **LocalCortex** macOS app, built with the AppleScript/JXA surface
  ...
  check needed.
- The **first call from the Codex host binary triggers a one-time macOS TCC
...
2. **`SKILL.md` frontmatter** — only `name` and `description`. The
   `description` is what Codex uses to decide when to load the skill, so make
   it specific about what the skill does and when it should trigger. Keep all
   trigger guidance in the description rather than relying on a body section.
...
9. **List the new skill** in both READMEs (`codex-plugin/README.md` and
   `codex-plugin/plugins/localcortex/README.md`) and bump the version in
   `codex-plugin/plugins/localcortex/.codex-plugin/plugin.json`.
...
6. **Update the plugin:** add the skill to both Codex READMEs' skill lists and
   bump the version in `plugins/localcortex/.codex-plugin/plugin.json`.

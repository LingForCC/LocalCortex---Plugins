...
recurring scheduled task will pick up future tasks.
...
delete the task, or ask Codex in the desktop app to stop it. Also tell them the scheduled task keeps firing
even when no agent has any active work left — idle ticks are silent no-ops
that create nothing, and it only stops when they pause or delete it from **Scheduled**. Also
...
> skill itself). If no supported agent has any active task, the tick does
...
Work **only one task per agent per tick**. Never touch a
...
create no task** — idle ticks are silent no-ops. A scheduled run **cannot pause
or delete its own schedule**, so the scheduled task simply keeps firing no-op
ticks until the user pauses or deletes it from **Scheduled** in the ChatGPT
desktop app. Skip step 5 entirely and stop.
...
first tick (already run at setup), and **how to stop it** (open **Scheduled** in the ChatGPT desktop app, then pause or delete the task). Also mention the **idle behavior**: once no
agent has any active task left, ticks become silent no-ops that create
nothing — the scheduled task keeps firing until the user pauses or deletes it. Finally note
...
worker) is all the trace there is.

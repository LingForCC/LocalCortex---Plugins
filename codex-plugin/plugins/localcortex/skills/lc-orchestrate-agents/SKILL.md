...
description: >-
  Set up recurring LocalCortex multi-agent delegation for a named Effort.
  Create a Codex scheduled task that polls every 5 minutes, re-reads the app's
  `list agents` roster, and spawns each supported agent CLI (opencode, kimi,
  codex, or claude code) that has open work. Read each agent's tool, model, and
  thinking effort from the app; never ask the user to supply that roster.
  Delegate one task id per eligible agent per tick to `lc-start-work`, in
  parallel, so each worker verifies, claims, works, and completes its own task.
  Drive LocalCortex through JXA/AppleScript (`osascript`), not MCP. Use when
  the user asks to orchestrate or schedule all configured agents on an Effort.
---

# lc-orchestrate-agents — a scheduled multi-agent delegation orchestrator

Set up a Codex scheduled task that, every 5 minutes, checks a named
...
| **codex** | ✅ `-m <model>` | ✅ `-c model_reasoning_effort=<supported value>` |
...
1. **At setup time (interactive, with the user):** collect the effort name and
   ...
   app** (`list agents`) and map each to a spawnable CLI; create the Codex
   scheduled task; then run the first tick immediately in the current session.
...
- **Codex Scheduled tasks** are available in the ChatGPT desktop app. The
  setup step creates one in the local project; tell the user it will fire
  every 5 minutes in the background until they pause or delete it. Codex CLI
  and the IDE extension cannot create or manage Scheduled tasks.
...
- **For codex agents**, models are Codex model strings available to the
  user's ChatGPT plan or `OPENAI_API_KEY` (`~/.codex/config.toml`). The
  model comes from each agent's `model` field; `thinking_effort` must be a
  `model_reasoning_effort` value supported by that model and the installed
  Codex CLI.
...
| `codex` | `cd "<CWD>" && codex exec -m <model> -c model_reasoning_effort=<effort> -s workspace-write -a never "<worker prompt>"` (`codex exec` runs headless to completion; model via `-m`; reasoning effort via `-c model_reasoning_effort=`; `-s workspace-write` lets the worker read/write the working dir; `-a never` is yolo — never ask for approval; cwd is set by `cd`, since `codex exec` otherwise requires the cwd to be a git repo. Omit `-m` / `-c …` when the agent's `model` / `thinking_effort` is empty) |
...
- **If the supported list is empty** (no agents defined, or none map to a known
  ...
  `claude code`.
  **Do not create the scheduled task.**

### Step 4 — Create the Codex scheduled task (every 5 minutes)

Use the host's scheduled-task capability to create a **standalone recurring
task** that runs every 5 minutes. Codex exposes Scheduled task creation and
management in the ChatGPT desktop app, not in Codex CLI or the IDE extension.
If this host cannot create Scheduled tasks, stop after validation and tell the
user to invoke this skill from Codex in the desktop app; do not claim a
schedule was created.

Configure the scheduled task as follows:

- **Schedule:** every 5 minutes, recurring.
- **Project:** the local project that contains the confirmed `<CWD>`.
- **Execution:** run in that local project directory, not an isolated worktree.
  The tick must reach LocalCortex through Apple Events and must spawn workers
  in the confirmed working directory.
- **Permissions:** use the narrowest unattended permission configuration that
  allows `osascript` and the configured worker CLIs to run. Scheduled tasks
  cannot pause for approvals. If current sandbox rules do not allow those
  commands, stop and tell the user what permission or command rule is needed;
  do not silently broaden access.
- **Title:** `lc-orchestrate-agents: poll <effort name> every 5 min for all supported agents`.
- **Prompt:** use the **exact** prompt block from
  [Scheduled run](#the-scheduled-run-each-tick-headless) below, with
  `<EFFORT_NAME>` and `<CWD>` filled in with the validated values. The
  prompt does **not embed agent ids**; it re-reads `agents-list` every tick,
  so app Settings changes take effect without recreating the scheduled task.
  Do not paraphrase it.

Record the scheduled task's identifier or title returned by the host so it can
be reported to the user.

### Step 5 — Run the first tick immediately

Do not wait ~5 minutes for the scheduled task's first fire. Right after creating
it, run one tick now, in this session: follow the
[Scheduled run](#the-scheduled-run-each-tick-headless) flow below exactly as
the scheduled task prompt will, with the validated values filled in. If no agent
...
scheduled task will pick up future tasks.
...
the cwd, that the scheduled task is recurring every 5 minutes, and that the first
tick has already run — report its outcome (which workers were spawned, or that
the effort was idle). **Tell the user how to stop it:** the scheduled task persists until paused
or deleted; they can open **Scheduled** in the ChatGPT desktop app and pause or
delete the task, or ask Codex in the desktop app to stop it. Also tell them: **when every supported
...
effort telling them to delete the scheduled task — completing that reminder does
NOT stop the scheduled task, they must still pause or delete it from **Scheduled**.** Also
remind them the spawned worker CLIs must stay logged in (`opencode auth login`,
`kimi login`, `codex login`, `claude auth login`) for ticks to do real work. Finally, tell them
...
on the next tick without recreating the scheduled task.
...
> block into the scheduled task's `prompt`. **Keep it self-contained** — a headless
...
task telling the user to stop the scheduled task (step 4 Branch B) — the scheduled task
...
Resolve the bundled helper from this skill's absolute path once and reuse
`$LC_JS`:
...
without recreating the scheduled task):
...
`blocked`), the effort is idle. Do not rely on a scheduled run to pause or delete its
own schedule. Instead, create
**one** reminder task in this effort telling the user to stop the scheduled task.
...
the **Scheduled** instructions so the user knows exactly how to stop it):

```
All supported agents (<names, comma-separated>) have no open, in_progress, or
blocked tasks in the '<EFFORT_NAME>' effort. The orchestrator scheduled task is
still running every 5 minutes and will keep firing no-op ticks until you delete
it — a scheduled run cannot stop itself.

To stop it: open Scheduled in the ChatGPT desktop app, find the task titled
"lc-orchestrate-agents: poll <EFFORT_NAME> every 5 min for all supported agents",
and pause or delete it (or ask Codex in the desktop app to stop it).

Completing this reminder does NOT stop the scheduled task — you must pause or
delete the scheduled task separately.
...
`-s workspace-write` so the worker can read/write the working dir, `-a never`
for yolo (never ask for approval), cwd set by `cd` (`codex exec` otherwise
requires the cwd to be a git repo):

```bash
cd "<CWD>" && codex exec -m <model> -c model_reasoning_effort=<effort> -s workspace-write -a never "<worker prompt for this agent>"
```
...
KIMI_PID=$!
# codex worker
( cd "<CWD>" && codex exec -m <model> -c model_reasoning_effort=<effort> -s workspace-write -a never "<codex prompt>" ) &
CODEX_PID=$!
# claude code worker
...
scheduled task easy to reason about.
...
- **At most one reminder per effort.** When the effort goes idle, the tick
  ...
  Completing or dismissing the reminder does **not** stop the scheduled task — the
  user must still delete it from **Scheduled** in the ChatGPT desktop app. The reminder is
...
first tick (already run at setup), and **how to stop it** (open **Scheduled** in the ChatGPT desktop app, then pause or delete the task). Also mention the **idle-reminder behavior**:
...
scheduled task; completing that reminder does not stop the scheduled task. Finally note
that **the agent roster is re-read every tick**, so editing agents in the app
takes effect on the next tick without recreating the scheduled task. During the

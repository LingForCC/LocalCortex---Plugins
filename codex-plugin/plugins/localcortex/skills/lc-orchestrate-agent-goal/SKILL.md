...
description: >-
  ...
  codex, claude code, copilot) with an open task, it picks one task and
  honors its `run_as`: `headless` (the default) spawns the agent's CLI
  headless; `subagent` runs the work as an in-session subagent via the
  spawn_agent tool when the agent's tool is this session's own CLI, else falls back to
...
**driver**. `lc-orchestrate-agents` creates a **Codex scheduled task**
...
in this session with the **spawn_agent tool**) — see
...
flow. This skill does **not** create or manage a Codex Scheduled task — there
is no scheduled automation in this flow at all.
...
- **`tool` → CLI mapping** (case-insensitive substring of the `tool` field):
  ...
  | `codex` | `codex` | headless, or in-session subagent via the **spawn_agent tool** on a `subagent` task |
  ...
  session). `codex` maps to **this session itself**: those agents dispatch
  through the **spawn_agent tool** when the picked task's `run_as` is `subagent` (see
  [Per-task run mode](#per-task-run-mode-run_as-headless-vs-subagent-read-this-first)).
- **`model`** and **`thinking_effort`** are applied to opencode, kimi, codex,
  claude code, and copilot headless spawns
  (see [Model & thinking-effort support](#model--thinking-effort-support-read-this-first)).
  For `codex` subagent dispatches they are stated in the subagent prompt as
...
five headless CLIs; the `codex` subagent row below carries them in the prompt
...
| **codex** (subagent) | ➖ stated in the subagent prompt (advisory; inherits this session's model) | ➖ stated in the subagent prompt (advisory) |
...
| `"subagent"` | `codex` — this session's own CLI | **in-session subagent** via the **spawn_agent tool** — the compatible case |
| `"subagent"` | `opencode` / `kimi` / `claude code` / `copilot` | **fall back to headless** — the compatibility rule |
...
`tool` **is the CLI this session is running as** — in this (Codex) plugin,
that means a `codex`-tool agent. Any other tool cannot run as a subagent of
...
tool would be that host's CLI; here it is `codex`.)

### How a subagent dispatch works

- Spawn it with the **spawn_agent tool** — a fresh agent with full tools, able to load and
...
- The user wants **unattended, recurring** delegation that keeps running in the
  ...
  which creates a 5-minute Codex scheduled task.
...
- **At least one supported agent is defined in the app.** Agents are created in
  ...
  copilot — a `codex` agent dispatches headless by default, or in-session via
  the **spawn_agent tool** on a `subagent` task). If no agent is defined, or none maps to a
...
- **The spawned worker CLIs are installed and logged in.** Headless workers
  ...
  relying on this
  orchestration. (In-session `codex` subagents need no login — they run
  inside this session.)
- **The `lc-start-work` skill is installed** for every spawned agent (in that
  ...
  (A `codex` subagent loads it from this same plugin's cache.)
...
it cannot run as a subagent of this session); `codex` additionally dispatches
**in-session**, via the **spawn_agent tool**, when the picked task's `run_as` is `subagent`.
...
- **On success** → `JSON.parse` the array. For each agent record, compute its
  CLI type from the `tool` field (case-insensitive substring):
    ...
    - **Supported agents** — one entry per agent whose `tool` mapped: `{id, name,
      ...
      A `codex`-type entry can also dispatch in-session (via the **spawn_agent tool**) when its
...
**Do not create a Codex scheduled task.** There is no scheduled job in this
flow. Instead,
...
5. on **dispatch**, spawn one worker per supported agent that has an `open`
   ...
   (headless CLI, or in-session subagent for a compatible `codex`
...
claude code (`--model` / `--effort`), copilot (`--model` / `--effort`); for
a `codex` subagent they ride in the prompt (see
...
(headless CLI, or in-session subagent for a compatible `codex` agent).
...
- `run_as` = `"subagent"`, agent type `codex` (this session's own CLI) → the
...
`codex`** — dispatch an **in-session subagent** with the **spawn_agent tool** (the only
compatible subagent case; every other agent type falls back to its headless
spawn below):

- the **spawn_agent tool** spawns a fresh subagent thread with full tools; it loads and runs the **`lc-start-work`** skill from this same plugin.
...
ones that have an `open` task this round; a `codex` agent dispatches
in-session on a `subagent` task):

First launch this round's subagents (spawn_agent calls — they run while the
bash block below executes):

```
spawn_agent(label="<AGENT_NAME> worker: <TASK_ID>",
            prompt=<codex subagent worker prompt>)   # block on its result later
```
...
**In-session subagent** (`codex` agent, `run_as` = `subagent` — the
...
- **Login is a prerequisite, not a round concern.** If a spawn fails because a
  ...
  `copilot login`)
  separately. (In-session `codex` subagents have no login — they run inside
  this session.)
...
logged in for their rounds to do real work (in-session `codex` subagents

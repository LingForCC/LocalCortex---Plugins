...
description: >-
  ...
  headless (`zcode` via the bundled app-server worker); `subagent` runs the work as an in-session subagent via the
...
- **`tool` → CLI mapping** (case-insensitive substring of the `tool` field):
  ...
  [Supported agents](#supported-agents)). `codex` maps to **this session itself**: those agents dispatch
...
six headless dispatches; the `codex` subagent row below carries them in the prompt
...
| `"subagent"` | `opencode` / `kimi` / `claude code` / `copilot` / zcode | **fall back to headless** — the compatibility rule |
...
- **At least one supported agent is defined in the app.** Agents are created in
  ...
  copilot / zcode — a `codex` agent dispatches headless by default, or in-session via

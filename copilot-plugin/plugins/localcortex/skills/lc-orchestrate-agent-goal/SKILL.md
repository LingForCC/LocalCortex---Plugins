...
description: >-
  ...
  headless; `subagent` runs the work as an in-session subagent via the task
...
**driver**. `lc-orchestrate-agents` creates a **Claude Code scheduled automation**
...
in this session with the **task tool** (spawning a custom agent)) — see
...
- **`tool` → CLI mapping** (case-insensitive substring of the `tool` field):
  ...
  | `copilot` | `copilot` | headless, or in-session subagent via the **task tool** on a `subagent` task |
  ...
  session). `copilot` maps to **this session itself**: those agents dispatch
  through the **task tool** when the picked task's `run_as` is `subagent` (see
  [Per-task run mode](#per-task-run-mode-run_as-headless-vs-subagent-read-this-first)).
- **`model`** and **`thinking_effort`** are applied to opencode, kimi, codex,
  ...
  For `copilot` subagent dispatches they are stated in the subagent prompt as
...
five headless CLIs; the `copilot` subagent row below carries them in the prompt
...
| **copilot** (subagent) | ➖ stated in the subagent prompt (advisory; inherits this session's model) | ➖ stated in the subagent prompt (advisory) |
...
| `"subagent"` | `copilot` — this session's own CLI | **in-session subagent** via the **task tool** — the compatible case |
| `"subagent"` | `opencode` / `kimi` / `codex` / `claude code` | **fall back to headless** — the compatibility rule |
...
`tool` **is the CLI this session is running as** — in this (Copilot CLI) plugin,
that means a `copilot`-tool agent. Any other tool cannot run as a subagent of
...
tool would be that host's CLI; here it is `copilot`.)

### How a subagent dispatch works

- Spawn it with the **task tool** (spawning a custom agent) — a fresh agent with full tools, able to load and
...
- The **LocalCortex** macOS app is installed and built with the AppleScript/JXA
  ...
  check needed.
- The **first call from the Claude Code host triggers a one-time macOS TCC
...
- **At least one supported agent is defined in the app.** Agents are created in
  ...
  copilot — a `copilot` agent dispatches headless by default, or in-session via
  the **task tool** on a `subagent` task). If no agent is defined, or none maps to a
...
- **The spawned worker CLIs are installed and logged in.** Headless workers
  ...
  relying on this
  automation. (In-session `copilot` subagents need no login — they run
  inside this session.)
- **The `lc-start-work` skill is installed** for every spawned agent (in that
  ...
  (A `copilot` subagent loads it from this same plugin's cache.)
...
it cannot run as a subagent of this session); `copilot` additionally dispatches
**in-session**, via the **task tool**, when the picked task's `run_as` is `subagent`.
...
```bash
# Resolve once. CLAUDE_PLUGIN_ROOT points at the plugin root (…/localcortex);
# the helper is under skills/lc-orchestrate-agent-goal/scripts/lc.js.
if [ -n "$CLAUDE_PLUGIN_ROOT" ]; then
  LC_JS="$CLAUDE_PLUGIN_ROOT/skills/lc-orchestrate-agent-goal/scripts/lc.js"
else
...
- **On success** → `JSON.parse` the array. For each agent record, compute its
  CLI type from the `tool` field (case-insensitive substring):
    ...
    - **Supported agents** — one entry per agent whose `tool` mapped: `{id, name,
      ...
      A `copilot`-type entry can also dispatch in-session (via the **task tool**) when its
...
**Do not create a Claude Code automation.** This skill does not use `CronCreate` /
...
5. on **dispatch**, spawn one worker per supported agent that has an `open`
   ...
   (headless CLI, or in-session subagent for a compatible `copilot`
...
a `copilot` subagent they ride in the prompt (see
...
(headless CLI, or in-session subagent for a compatible `copilot` agent).
...
- `run_as` = `"subagent"`, agent type `copilot` (this session's own CLI) → the
...
`copilot`** — dispatch an **in-session subagent** with the **task tool** (spawning a custom agent) (the only
compatible subagent case; every other agent type falls back to its headless
spawn below):

- the **task tool** spawns a custom agent — a fresh agent with full tools; it loads and runs the **`lc-start-work`** skill from this same plugin.
...
ones that have an `open` task this round; a `copilot` agent dispatches
in-session on a `subagent` task):

First launch this round's subagents (task-tool calls — they run while the
bash block below executes):

```
task tool: spawn a general-purpose custom agent
  name: "<AGENT_NAME> worker: <TASK_ID>"
  prompt: <copilot subagent worker prompt>
```
...
**In-session subagent** (`copilot` agent, `run_as` = `subagent` — the
...
- **Login is a prerequisite, not a round concern.** If a spawn fails because a
  ...
  `copilot login`)
  separately. (In-session `copilot` subagents have no login — they run inside
  this session.)
...
logged in for their rounds to do real work (in-session `copilot` subagents

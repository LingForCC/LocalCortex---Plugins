...
- **`tool` → CLI mapping** (case-insensitive substring of the `tool` field):
  ...
  [Supported agents](#supported-agents)). `copilot` maps to **this session itself**: those agents dispatch
...
six headless dispatches; the `copilot` subagent row below carries them in the prompt
...
| `"subagent"` | `opencode` / `kimi` / `codex` / `claude code` / zcode | **fall back to headless** — the compatibility rule |
...
- **At least one supported agent is defined in the app.** Agents are created in
  ...
  copilot / zcode — a `copilot` agent dispatches headless by default, or in-session via

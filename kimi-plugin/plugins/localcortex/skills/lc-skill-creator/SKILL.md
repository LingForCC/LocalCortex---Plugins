...
description: >-
  Create or revise a Kimi Code skill that drives the LocalCortex macOS task
  ...
  need.
whenToUse: >-
  When the user wants to build a new lc-* skill that automates a LocalCortex
  workflow the existing skills don't cover, extend or fix an existing lc-*
  skill, or learn the JXA surface to hand-roll one-off automation.
arguments: request
---
...
2. **`SKILL.md` frontmatter** — `name`, `description` (keep under 1024 chars),
   `whenToUse`, and `arguments` (space-separated argument names, e.g.
   `effort task`). The `description` is what the host uses to decide when to
   load the skill — make it specific about *when to use* and *when not to
   use*.
...
9. **List the new skill** in both READMEs (`kimi-plugin/README.md` and
   `kimi-plugin/plugins/localcortex/README.md`) and bump the plugin version in
   `plugins/localcortex/kimi.plugin.json` (the kimi `marketplace.json` carries
   no per-plugin version).
...
6. **Update the plugin:** add the skill to both READMEs' skill lists, and bump
   the version in `plugins/localcortex/kimi.plugin.json`.

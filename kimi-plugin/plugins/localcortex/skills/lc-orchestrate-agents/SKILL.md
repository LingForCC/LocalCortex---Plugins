...
recurring cron job will pick up future tasks.
...
(`CronDelete`), or ask you to stop it. Also tell them the job keeps firing
even when no agent has any active work left — idle ticks are silent no-ops
that create nothing, and the job only stops when they delete it. Also
...
create no task** — idle ticks are silent no-ops. The cron job **cannot delete
or disable itself** — a scheduled run is blocked from `CronCreate` /
`CronDelete` by the host (only `CronList` is allowed) — so it simply keeps
firing no-op ticks until the user deletes it. Skip step 5 entirely and stop.
...
delete by id with `CronDelete`). Also mention the **idle behavior**: once no
agent has any active task left, ticks become silent no-ops that create
nothing — the job keeps firing until the user deletes it. Finally note

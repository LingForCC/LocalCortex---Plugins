...
whenToUse: >-
  When the user explicitly points to a LocalCortex task (by id or name) and
  asks to start it, work on it, pick it up, resume it, or find it before
  doing the work.
arguments: task
---
...
Implement the LocalCortex task workflow described below for the task the user
named: $task

Drive LocalCortex **exclusively through its JXA/AppleScript surface** via the
bundled `lc.js` helper — never use `mcp__localcortex__*` tools in this
skill's flow.
...
```bash
LC_STATUS=in_progress LC_WORKER=agent LC_WORKER_LABEL=kimi \
  osascript -l JavaScript "$LC_JS" task-update "$TASK_ID"
```

Then **re-read the task** (`tasks-get`) and confirm its `worker` is `agent`
**and** `worker_label` is `kimi`. If it isn't, **stop and double-check with
...
Follow-up: write integration tests (owner: agent/kimi).' \
  osascript -l JavaScript "$LC_JS" task-update "$TASK_ID"
```

...
description: >-
  ...
  headless; `subagent` runs the work as an in-session subagent when the
...
- **`tool` → CLI mapping** (case-insensitive substring of the `tool` field):
  ...
  session). `kimi` maps to **this session itself**: those agents dispatch
...
five headless CLIs; the `kimi` subagent row below carries them in the prompt
...
| `"subagent"` | `opencode` / `codex` / `claude code` / `copilot` | **fall back to headless** — the compatibility rule |
...
- **At least one supported agent is defined in the app.** Agents are created in
  ...
  copilot — a `kimi` agent dispatches headless by default, or in-session via

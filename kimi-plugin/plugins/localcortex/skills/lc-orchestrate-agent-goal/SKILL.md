...
description: >-
  ...
  manager app — that loops in the current session (no cron job)
...
whenToUse: >-
  ...
  fresh headless workers, looping until no supported agent has any active task
  left and stopping on its own, with no cron job created.
arguments: effort
---

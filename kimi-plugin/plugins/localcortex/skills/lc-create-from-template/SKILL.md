...
description: >-
  ...
  agent by id — the only assignment wire, since 0.3.11 removed human
  assignments. When a task is Blocked, the status and its blocker list
...
whenToUse: >-
  When the user wants to populate a LocalCortex Effort from a task Template —
  taking the template's free-text prompt and turning it into the described
  tasks (roots, subtasks, assignments, blockers) inside a chosen effort.
arguments: effort template
---
